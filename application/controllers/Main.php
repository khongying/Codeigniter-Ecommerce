	...
	public function add_order(){
		...
		if ($query) {
			foreach ($data['code'] as $key => $value) {
				...
				$code_qty = $this->product_m->get_product_qty($data['code'][$key]);
				$qty = $code_qty[0]->total;

				$updata_product = array(
					'total' => ($qty-$data['qty'][$key])
				);

				$this->db->where("code", $data['code'][$key]);
        		$query = $this->db->update("product",$updata_product);
